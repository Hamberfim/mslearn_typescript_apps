...
let someRandomValue: any;
someRandomValue = 10;
someRandomValue = 'pizza';

let otherRandomValue: unknown;
...
console.log((otherRandomValue as string).toUpperCase());
...
// literal types - number
type coinSide = 0 | 1;
let coinToss: coinSide = Math.random() < 0.5 ? 0 : 1;
function coinFlip(coinToss: coinSide) {
  if (coinToss === 0) {
    console.log('tails');
  } else {
    console.log('heads');
...
// collection types
let myList: number[] = [3, 2, 1]; // array
let person: [string, number] = ['Dave', 44]; // tuple
